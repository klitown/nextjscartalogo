...
export const Carrito = ({ tienda }: any) => {
    ...
    const generarMensaje = (telefono: number, productos: any[], totalCarrito: number) => {
        let apiWhatspp = `https://api.whatsapp.com/send/?phone=${telefono}&text=`;
        let texto = `Hola!%20vi%20${productos.length === 1 ? 'este' : 'estos'}%20${productos.length === 1 ? 'producto' : 'productos'}%20en%20la%20web
    %20y%20quiero%20confirmar%20el%20pedido:%0A`;
        let nombresProductos = productos.map(producto => producto.nombre + ' ' + `(Cantidad: ${producto.quantity ? producto.quantity : 1}).` + '%0A');
        let replaced = nombresProductos.join('').replace(/ /g, '%20');
        let final = apiWhatspp + texto + replaced + '%0A' + `${totalCarrito ? `El%20total%20según%20la%20web%20es%20de%20${formatNumber(totalCarrito)}%20.` : ''}`;
        return final;
    }
            ...
            <div className="min-h-screen">
                ...
                <div className="flex justify-start items-center mx-auto ml-10">
                    <Link href={`/${tienda.url}`}
                        ...
                        <span className="text-white font-bold">Volver a la tienda</span>
                    </Link>
                </div>
                ...
                <div className="flex w-full lg:px-20 xl:px-32">
                    <div className="px-4 pt-8 w-full">
                        ...
                        <p className="text-gray-400">
                            Por favor, revisa los productos elegidos antes de proceder a la compra
                        </p>
                        <div className="mt-8 space-y-3 rounded-lg border bg-white px-2 py-4 sm:px-6">
                            {
                                items.length >= 1 ?
                                    <>
                                        {
                                            items.map(prod => {
                                                const priceFormatted = parseInt(`${prod.price}`, 10);
                                                return (
                                                    <div className="flex flex-col rounded-lg bg-white sm:flex-row border border-gray-200 p-4" key={prod.id}>
                                                        <div className="w-full p-2">
                                                            <Image src={prod.imagenes.length >= 1
                                                                ? prod.imagenes[0]
                                                        ...
                                                        <div className="flex w-full flex-col px-4 py-4">
                                                            <span className="font-semibold text-xl text-black">
                                                                {prod.nombre} ({prod.quantity} {prod.quantity === 1 ? 'unidad' : 'unidades'} )
                                                            </span>
                                                            <p className="text-lg font-bold">
                                                                Gs {priceFormatted.toLocaleString("es-ES")}
                                                            </p>
                                                            ...
                                                            focus:shadow bg-red-500 hover:bg-red-800">
                                                                ...
                                                                </svg> <span className="text-white font-bold">
                                                                    Eliminar
                                                                </span>
                                                            </button>
                                                        </div>
                                        ...
                                        <div className="flex justify-center items-center mt-10">
                                            <button onClick={($event) => {
                                                ...
                                                let url = generarMensaje(tienda.telefono, items, cartTotal);
                                    ...
                                    </> : <h1>Empieza a comprar para ver los productos!</h1>
                            }
